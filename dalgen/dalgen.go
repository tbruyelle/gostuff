...
func main() {
	...
	for _, d := range g.pkg.defs {
		for _, d := range f.file.Unresolved {
			fmt.Println(d.Name, d.Obj, d.String(), d.Pos())
			if d.Name == typeName {
				if !d.IsExported() {
					log.Fatalf("%s must be exported", typeName)
				}
				fmt.Printf("%+v\n", g.pkg.defs[d])

				if d.Obj.Kind != ast.Typ {
					log.Fatalf("%s must be a type", typeName)
				}
				switch s := d.Obj.Type.(type) {
				case ast.StructType:
					for _, field := range s.Fields.List {
						fmt.Println("Hello field", field)
					}
				default:
					log.Fatalf("%s must be a struct", typeName)
				}
			}
		}
	}
